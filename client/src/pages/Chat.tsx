      ...
      ws.onmessage = (response) => {
        if (typeof response.data === "string") {
          ...
          if (data.type === "response") {
            setChats((prev) => [...prev, { sender: "server", message: data.payload.message }]);
          }
          if (data.type === "error") {
      ...
      <div>
        <div className="border-white/10 border-2  rounded-lg mx-auto max-w-xl h-[45rem] flex flex-col gap-2 p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-white text-xl font-medium ">Real Time Chat</div>
          </div>

          <div className="text-white/60  font-thin  text-[13px]">temporary room that expires after both users exit</div>

          {/* Room details */}
          <div className="bg-white/10 rounded-md p-2 flex justify-between items-center my-5 border-white/10 border px-2">
            <div className="text-white/60 text-[12px] font-thin">Room Code :</div>
              ...
              {chats.map((e, index) => (
                <div
                  ...
                  className={`min-h-0 p-2 w-fit rounded-md text-sm   ${
                    e.sender === "server" ? "bg-white/50 self-start" : "bg-white self-end"
                  }`}
                >
                  {e.message}
                </div>
          ...
          <div className="flex gap-2 items-center">
            <input
              ...
              ref={inputRef}
            />
            <button onClick={sendMessage} className="bg-white h-10 w-20 rounded-md text-sm ">
              Send
            </button>
          </div>
        </div>
